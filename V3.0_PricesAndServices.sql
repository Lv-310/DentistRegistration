...
UPDATE PRICES
SET PRICE = @PRICE
WHERE DATE_START_PRICE = @DATE_START_PRICE AND SERVICE_ID = @SERVICE_ID
END;
...
IF object_id('spDeletePrice') IS NULL
    EXEC ('create procedure dbo.spDeletePrice as select 1')
GO
...
IF object_id('spCheckPriceForDelete') IS NULL
